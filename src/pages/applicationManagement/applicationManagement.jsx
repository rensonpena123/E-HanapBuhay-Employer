...
const ApplicationManagement = () => {
  const { applicants, loading, error } = useApplications();
  ...
  return (
    <section className="flex justify-center p-4">
      <div className="w-full max-w-7xl flex flex-col gap-6">
        ...
        <div className="bg-brand-dark rounded-3xl p-6 shadow-xl">
          <h1 className="text-3xl font-bold text-white mb-8">Application Management</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard title="New Applicants" value="248" icon={Briefcase} />
            <StatCard title="Shortlisted" value="142" icon={XCircle} />
            <StatCard title="Pending" value="38" icon={Clock} />
            <StatCard title="Total Hired" value="15" icon={BookMarked} />
          </div>
        </div>
           ...
           <FilterBar onClear={() => console.log("Filters cleared")}>
            <FilterItem label="Range Date">
              <div className="flex items-center gap-3 h-full">
                <input type="date" className={`${inputClass} w-40 uppercase`} />
