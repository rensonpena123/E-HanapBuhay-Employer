...
// key prop on the img resets the element entirely when src changes, avoiding stale imgFailed
const AvatarDisplay = ({ avatarSrc, user }) => {
  const [imgFailed, setImgFailed] = useState(false);

  // Reset failure state any time the src URL changes (e.g. new photo uploaded)
  useEffect(() => {
    setImgFailed(false);
  }, [avatarSrc]);
  ...
  // Fallback — initials avatar when no photo exists or image fails to load
  return <DefaultAvatar user={user} />;
};

const Header = () => {
  const [isMenuOpen,  setIsMenuOpen]  = useState(false);
  // currentUser — live state, re-read from localStorage on profileUpdated event
  ...
  // handleLogout — wipe auth data then redirect to login
  const handleLogout = () => {
  ...
  // roleLabel — always "Employer" for employer accounts
  const roleLabel = (currentUser.role === 'employer' || currentUser.role_id === 2)
    ? 'Employer'
    : currentUser.role || 'Employer';

  // avatarSrc — full URL or null; null renders DefaultAvatar (initials)
    ...
    <header className="h-20 bg-brand-dark flex items-center justify-between px-8 border-b-2 border-l-2 border-brand-yellow/80 shrink-0 relative z-20">
      ...
      <div className="flex items-center h-full gap-4">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {/* Dropdown — only My Profile and Sign Out, no repeated user summary */}
          {isMenuOpen && (
